 ...
 *
 * Es una pista, no la verdad: el estado que vale es el que devuelve
 * nuestra API, que lo consulta contra MercadoPago.
 */
      ...
      <Card>
        ...
        {escenario && (
          <p className="text-base text-foreground/60 mb-8">
            {escenario === 'exito'
              ? 'MercadoPago nos redirigió como pago aprobado. Si te llegó el comprobante, escribinos y verificamos el turno.'
              : escenario === 'pendiente'
                ? 'MercadoPago nos redirigió con el pago en proceso.'
                : 'MercadoPago nos redirigió como pago rechazado.'}
          </p>
        )}
        <Actions retry />
      </Card>
...
export function ResultadoPagoTurno({ escenario }: { escenario?: EscenarioRetorno }) {
  return (
